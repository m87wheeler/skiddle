...
import { LikedContainer } from "./styles";

interface Props {
  likes?: number;
}

const Liked = ({ likes = 0, ...props }: Props) => {
  const [liked, setLiked] = React.useState(0);

  // TODO hook up to API (POST)
  const handleLike = React.useCallback(() => {
    setLiked((state) => (state ? 0 : 1));
  }, []);
